...
# With this script the validation instructions for the varex-simulations are generated.
 ...
 if [ "$#" -eq 1 ]; then
  ...
  qos=''
 #account='--account=proj-dutch_scen'
 #qos='--qos=proj-dutch_scen_4'
  ...
  # Define the varex navigation aliases (those might be defined in your .bashrc as well):
  #for k in {h,i,j,s,t,u}; do
  # for j in $(seq -f "%02g" 1 16); do
  #  for i in {0..9}; do
     alias ${k}${j}${i}s="cd /scratch/ms/nl/nklm/submit-ec-earth-3/branch-varex-${k}${j}${i}"
     alias ${k}${j}${i}r="cd /scratch/ms/nl/nklm/ec-earth-3/branch-varex/${k}${j}${i}"
     alias ${k}${j}${i}c="cd /scratch/ms/nl/nklm/cmorisation/cmorised-results/cmor-VAREX-cmip-${k}${j}${i}/${k}${j}${i}/CMIP6/*MIP/KNMI/EC-Earth3/*/r*i*p5f1/"
     alias ${k}${j}${i}l="cd /scratch/ms/nl/nklm/cmorisation/cmorised-results/cmor-VAREX-cmip-${k}${j}${i}/${k}${j}${i}/logs/"
  #  done
  # done
  #done
 ...
 #for k in {h,i,j,s,t,u}; do
  ...
  #for j in $(seq -f "%02g" 1 16); do
   ...
   # for i in {5..9}; do
     ...
     ec_earth_dir=/scratch/ms/nl/nklm/ec-earth-3/branch-varex/${experiment_id}/output/
     filenumber_check_file=filenumber-check/filenumber-check-varex-simulations-${experiment_id}.txt
     cmorised_cmip6_dir=/scratch/ms/nl/nklm/cmorisation/cmorised-results/cmor-VAREX-cmip-${experiment_id}/${experiment_id}/CMIP6/
    ...
    #echo " for i in \`ls -1 -d ${table_dir}/*/*\`; do echo ../data-qa/scripts/files-per-variable.sh ${version} \${i}; done                                   " | sed 's/\s*$//g' >> ${output_file}
    #echo " for i in \`find ${table_dir} -type d -name ${version}\`; do ls -1 \${i} | wc -l | grep -v -e ${time_records} -e 1  && echo \${i}; done            " | sed 's/\s*$//g' >> ${output_file}
   ...
   ##echo " find ${cmorised_cmip6_dir} -regextype sed ! -regex '.*/.*_EC-Earth3*_[0-9a-zA-Z]*_${ripf}_g[nr][_0-9-]*\.nc -type f'                              " | sed 's/\s*$//g' >> ${output_file}
   ...
   ##echo " find ${cmorised_cmip6_dir} -regextype sed ! -regex '.*/.*_EC-Earth3*_[0-9a-zA-Z]*_${ripf}_g[nr][_0-9-]*\.nc -type f'                              " | sed 's/\s*$//g' >> ${output_file}
     ...
     echo "                                                                                                                                                   " | sed 's/\s*$//g' >> ${output_file}
#     echo " # Removing the ${experiment_id} raw output after all cmorised data has been succesfuly checked:                                                   " | sed 's/\s*$//g' >> ${output_file}
#     echo " cd ${ec_earth_dir}                                                                                                                                " | sed 's/\s*$//g' >> ${output_file}
#     echo " ls -d  ${remove_pattern}                                                                                                                          " | sed 's/\s*$//g' >> ${output_file}
#     echo " rm -rf ${remove_pattern}                                                                                                                          " | sed 's/\s*$//g' >> ${output_file}
     echo "                                                                                                                                                   " | sed 's/\s*$//g' >> ${output_file}
     echo "                                                                                                                                                   " | sed 's/\s*$//g' >> ${output_file}
...
# make second validation file, to check after the jobs are done that are submitted by the first validation file (checksum and nctime)
...
##     echo " chmod -R uog-w ${cmorised_cmip6_dir}/../                                                                                                          " | sed 's/\s*$//g' >> ${output_file2}
